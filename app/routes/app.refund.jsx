...
export default function RefundPage() {
  ...
  const data = {
    ...
    selectedOrder: fetcher?.data?.selectedOrder || initialSelectedOrder,
  };

  const selectedOrder = searchParams.get("orderId") ? (fetcher?.data?.selectedOrder || initialSelectedOrder) : null;
  ...
  useEffect(() => {
    ...
    if (selectedOrder?.id !== prevOrderIdRef.current) {
      console.log("Resetting states for new order:", selectedOrder?.id);
  ...
  const handleRefund = async () => {
    ...
    try {
      if (paymentMode === 'paypal') {
        ...
        payload.variables.input.note = `Refunded via PayPal: ${data.paypalRefundId} at 12:46 PM IST on 02/06/2025`;
        const formData = new FormData();
        ...
        const payload = preparePayload();
        payload.variables.input.note = `Refunded via Stripe: ${data.stripeRefundId} at 12:46 PM IST on 02/06/2025`;
        const formData = new FormData();
        formData.append("body", JSON.stringify({ ...payload, mode: "refund" }));
        fetcher.submit(formData, { method: "POST" });
        ...
        const payload = preparePayload();
        payload.variables.input.note = `Refund processed via app at 12:46 PM IST on 02/06/2025`;
        const formData = new FormData();
        formData.append("body", JSON.stringify({ ...payload, mode: "refund" }));
        fetcher.submit(formData, { method: "POST" });
      }

      // Reset all states after refund
